...
class AgentComm(object):
    ...
    # - sock.close() +
    # - sock.connect(url) +
    # sock.setLinger(-1) -> infinite linger (try send after close() was called)
    # - sock.Socket(AF_SP, NN_PULL|NN_PUSH)
...
def parse_args():
    ...
    argparser.add_argument("--proto", "-p", dest="use_proto",
                           action="store_true")
    argparser.add_argument("--json", "-j", dest="use_json",
                           action="store_true")
...
def main():
    ...
    print('Use Proto:', use_proto, 'Use JSON:', use_json)
    ...
    with timers.timing("Writing output"):
        with open(output_file, "w") as output_file:
            if use_proto:
                ...
                sas_task.output_json(output_file)
            else:
